...
# Input validation
if [ -z "$1" ]; then
    echo "Error: Please provide the Jenkins version as an argument (e.g., 2.361.1)."
    exit 1
fi

if [ -z "$2" ]; then
    echo "Error: Please provide the OS type (e.g., ubuntu, centos, macos)."
    exit 1
fi
...
install_java() {
    if [ "$OS_TYPE" == "ubuntu" ] || [ "$OS_TYPE" == "debian" ]; then
        ...
        echo "Error: Unsupported OS type. Please provide either 'ubuntu', 'centos', or 'macos'."
        exit 1
    fi
}
...
download_jenkins() {
    ...
    if ! wget https://get.jenkins.io/war-stable/$JENKINS_VERSION/jenkins.war -O $JENKINS_INSTALL_DIR/$JENKINS_WAR_FILE; then
        echo "Error: Failed to download Jenkins WAR file."
        exit 1
    fi
}
...
start_jenkins() {
    ...
    if ! java -jar $JENKINS_INSTALL_DIR/$JENKINS_WAR_FILE --httpPort=8080; then
